...
class Stakehouses:
    ...
    async def get_knot_details(self, validator_ids, order_by=None, order_direction='asc'):
        ...
        query = GraphQLRequest(
            query=query,
            variables={'ids': validator_ids, 'order_by': order_by, 'order_direction': order_direction}
        )
...
class ISavETHManager:
    ...
    def approve_deth(self, deth_required):
        deth_contract = self.deth_contract.functions.approve(
            SAV_ETH_MANAGER_CONTRACT_ADDRESS, self.web3.toWei(deth_required, 'ether')).buildTransaction()
        tx_receipt = self.web3.eth.sendRawTransaction(
            self.web3.eth.account.signTransaction(deth_contract, os.environ.get('PRIVATE_KEY')).rawTransaction)
...
class Arbitrage:
    ...
    def execute_arbitrage(self, 
        ...
        # Sign transaction
        signed_txn = self.sign_transaction(message, os.environ.get('PRIVATE_KEY'))
...
async def main():
    ...
    # open_index_validatos_with_rewards = [lsd_validator for lsd_validator in await lsd_validators_instance.get_lsd_validators(open_index_id, status=status)]
    ...
    l = min((len(openindex_validators_details), len(user_lsd_validators_details)))
    ...
    for i in range(l):
        open_index_deth_needed = i_save_eth_manager_instance.calc_deth_required(openindex_validators_details[i]['id'])
        openindex_validators_details[i]['deth_needed'] = open_index_deth_needed

        user_deth_needed = i_save_eth_manager_instance.calc_deth_required(user_lsd_validators_details[i]['id'])
        ...
        if open_index_deth_needed < user_deth_needed and openindex_validators_details[i]['reportedYield'] > user_lsd_validators_details[i]['reportedYield']:
            ...
            openIndex_validators_ids.append(openindex_validators_details[i]['id'])
            openIndex_validators_stakehouse_ids.append(Web3.toChecksumAddress(openindex_validators_details[i]['stakeHouse']))
            user_lsd_validators_ids.append(user_lsd_validators_details[i]['id'])
            user_lsd_validators_stakehouse_ids.append(Web3.toChecksumAddress(user_lsd_validators_details[i]['stakeHouse']))
    ...
    if consider_execution_cost and (deth_required_to_isolate + executing_cost) > deth_gained_for_returning:
